...
def train_svm(training, **kwargs):
    # Extract features and target variable from training data
    X_train = training[:, :-1]
    y_train = training[:, -1]
    
    # Initialize SVM model
    svm_model = SVR(**kwargs)
    
    return svm_model

def hyper_tune(data):
    results = {}
    for i,param in enumerate(dt_params):
        model = train_dt(data,**param)
        score = cross_val_score(model,data[:,0:-1],data[:,-1],cv=10,scoring="r2")
    ...
    for i,param in enumerate(svm_params):
        model = train_svm(data,**param)
        score = cross_val_score(model,data[:,0:-1],data[:,-1],cv=10,scoring="r2")
        print(i)
        results[f"svm_{i}"]=score.mean()
